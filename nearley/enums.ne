...
const ABILITY_WORDS = new Set(["adamant","addendum","battalion","bloodrush","channel","chroma","cohort","constellation","converge","coven","delirium","domain","eminence","enrage","exhaust","ferocious","formidable","grandeur","hellbent","heroic","imprint","inspired","kinship","landfall","lieutenant","metalcraft","morbid","parley","radiance","raid","rally","revolt","strive","sweep","temptingoffer","threshold","undergrowth"]);
const SIMPLE_KEYWORDS = new Set(["deathtouch","defender","flash","flying","haste","hexproof","indestructible","intimidate","lifelink","reach","shroud","trample","vigilance","flanking","phasing","shadow","horsemanship","fear","provoke","storm","sunburst","epic","convoke","haunt","delve","gravestorm","changeling","conspire","persist","wither","retrace","exalted","cascade","rebound","infect","undying","soulbond","unleash","cipher","evolve","extort","fuse","dethrone","prowess","exploit","menace","devoid","ingest","myriad","skulk","melee","undaunted","improvise","aftermath","ascend","assist","mentor","riot","partner"]);
const COST_KEYWORDS = new Set(["equip","escape","spectacle","eternalize","embalm","escalate","emerge","surge","awaken","dash","outlast","mutate","bestow","scavenge","overload","buyback","echo","flashback","madness","morph","entwine","ninjutsu","transmute","replicate","recover","fortify","evoke","unearth","miracle","megamorph","prowl","transfigure","multikicker","disguise","plot","squad","reconfigure"]);
const NUMBER_KEYWORDS = new Set(["afterlife","afflict","fabricate","crew","renown","tribute","rampage","fading","amplify","modular","bushido","dredge","graft","ripple","vanishing","absorb","poisonous","devour","annihilator","frenzy","soulshift","hideaway"]);
const objHasMods = (o) => o?.prefixes||o?.suffix||o?.condition||o?.without||o?.object?.prefixes||o?.object?.suffix;
const flattenAbilities = (a) => {
  if (a == null) return [];
  if (Array.isArray(a)) return a.flatMap(flattenAbilities);
  if (a && typeof a === "object" && Array.isArray(a.and)) return a.and.flatMap(flattenAbilities);
  return [a];
};
const abilitiesFromWithClause = (withClause) => {
  const inner = withClause && withClause.with;
  if (inner && inner.ability !== undefined) return flattenAbilities(inner.ability);
  return null;
};
