...
connected[rule] -> $rule ("," __ $rule):* ",":? __ connector __ $rule
  {%
    ([[o], os, , , connector, , [o2]]) => ({
      [connector]: [o, ...os.map(([, , [o3]]) => o3), o2],
    })
  %}
...
activatedAbility -> costs ":" __ effect (("." __ | __ ) activationInstructions):?
  {%
    ([costs, , , activatedAbility, i]) => {
...
activatedAbilities -> (itsPossessive __):? "activated abilities"
  ...
  {%
    ([, , activatedAbilities]) => ({
      whose: activatedAbilities,
      activatedAbilities: true,
    })
  %}

activatedAbilitiesVP -> "can't" __ "be activated" (__ "unless they're mana abilities."):?
  {%
    ([, , , , manaOnly]) =>
      manaOnly
...
triggerCondition -> ("when" | "whenever") __ triggerConditionInner (__ triggerTiming):?
  {%
    ([, , inner, timing]) => {
...
sentenceDot -> sentence (".":? __ additionalSentence):* ".":?
  {% ([s, ss]) => (ss.length > 0 ? [s, ...ss.map(([, , s2]) => s2)] : s) %}
...
sentence -> singleSentence {% ([ss]) => ss %}
  | singleSentence ("," __ singleSentence):* ",":? __ connector __ singleSentence
    {%
      ([s1, ss, , , connector, , s2]) => {
        ...
        if (
          elements[0] &&
          elements[0].actor &&
          elements.slice(1).every((e) => !e || (!e.actor && !e.what))
        ) {
        ...
        if (elements[0] && elements[0].condition && elements[0].effect) {
          return {
            condition: elements[0].condition,
            effect: { [connector]: [elements[0].effect, ...elements.slice(1)] },
          };
        }
        // Propagate "for each" if first element has forEach+effect
        if (elements[0] && elements[0].forEach && elements[0].effect) {
          return {
            forEach: elements[0].forEach,
            effect: { [connector]: [elements[0].effect, ...elements.slice(1)] },
          };
        }
    ...
    %}
  | singleSentence ("," __ singleSentence):* "," __ "then" __ singleSentence
    {%
      ([s1, ss, , , , , s2]) => {
        const elements = [s1, ...ss.map(([, , s]) => s), s2];
        if (elements[0] && elements[0].condition && elements[0].effect) {
          return {
            condition: elements[0].condition,
            effect: { and: [elements[0].effect, ...elements.slice(1)] },
          };
        }
        return { and: elements };
      }
  ...
  | singleSentence __ "instead of" __ singleSentence
    {% ([does, , , , insteadOf]) => ({ does, insteadOf }) %}
...
singleSentence -> imperative {% ([i]) => i %}
  ...
  | "if" __ object __ "would" __ (objectVerbPhrase | objectInfinitive) "," __ sentenceInstead
    {% ([, , what, , , , [does], , , instead]) => ({ what, does, instead }) %}
  | "if" __ player __ "would" __ playerVerbPhrase (__ exceptClause):? (__ whileClause):? "," __ sentenceInstead
    {%
      ([, , actor, , , , would, except, whileC, , , instead]) => {
  ...
  | activatedAbilities __ activatedAbilitiesVP (__ duration):?
    {%
      ([abilities, , effect, duration]) =>
        duration ? { ...abilities, ...effect, duration: duration[1] } : { ...abilities, ...effect }
    %}
  | itsPossessive __ numericalCharacteristic __ ("is" | "are each") __ "equal to" __ numberDefinition
    {% ([what, , characteristic, , , , , , setTo]) => ({ what, characteristic, setTo }) %}
  | "the flashback cost is equal to" __ itsPossessive __ "mana cost"
    {% ([, , whose]) => ({ flashbackCost: { whose } }) %}
  | "as" __ sentence "," __ sentence {% ([, , as, , , does]) => ({ as, does }) %}
  | "instead" __ singleSentence {% ([, , instead]) => ({ instead }) %}
  | imperative __ "instead" __ "if" __ condition
    {% ([instead, , , , , , condition]) => ({ instead, condition }) %}
...
forEachClause -> "for each" __ pureObject {% ([, , forEach]) => ({ forEach }) %}
  | "for each color of mana spent to cast" __ object
    {% ([, , forEachColorSpent]) => ({ forEachColorSpent }) %}

condition -> sentence {% ([s]) => s %}
  # | ("you've" | "you") __ action __ duration  # temporarily removed — unused
  | "you've" __ "cast" __ countableCount __ object __ duration
    {%
      ([, , , , count, , what, , during]) => ({
        done: { cast: { count, what } },
        during,
      })
    %}
  ...
  | object __ "was kicked with its" __ manacost __ "kicker"
    {% ([what, , , , mana]) => ({ what, kicked: { with: { mana } } }) %}
  | object __ "has" __ englishNumber __ counterKind __ "counter" _s " on it"
    {% ([what, , , , amount, , counterKind]) => ({ what, has: { amount, counterKind } }) %}
  | "there" __ ("are" | "is") __ countableCount __ object __ inZone
    {% ([, , , , count, , what, , zone]) => ({ count, what, ...zone }) %}
  | "it's" __ "the" __ ordinal __ object __ player __ "cast" __ duration
    {%
      ([, , , , ordinal, , what, , actor, , , , during]) => ({
        ordinal,
        cast: { what, actor },
        during,
      })
...
# pureObject connector pureObject removed — types merge via anyType or plural pureObject1 rules
  ...
  | (counterKind __):? "counter" _s __ "on" __ object
    {%
      ([kind, , , , , , countersOn]) =>
        kind ? { counterType: kind[0], countersOn } : { countersOn }
    %}

suffix -> player __ (("don't" | "doesn't") __):? ("control" | "own") _s
  {%
    ([actor, negate, , [does]]) =>
...
pureObject1 -> (prefix __):* (anyType __):? pureObjectInner
  {%
    ([prefixes, types, object, suffix]) => {
  ...
  %}
  | (prefix __):* anyType "s" __ ("and" {% () => "and" %} | "or" {% () => "or" %} | "and/or" {% () => "or" %}) __ (prefix __):* anyType "s"
...
commonReferencingPrefix -> countableCount (__ "additional"):? (__ commonReferencingPrefixInner):?
  {%
    ([count, additional, inner]) => {
      const result = { count };
  ...
  %}
  | ("another" __):? (countableCount __):? "target" _s
  {%
    ([another, count]) => {
...
imperative -> "sacrifice" _s __ object {% ([, , , sacrifice]) => ({ sacrifice }) %}
  ...
  | "return" _s __ object  (__ fromZone):? __ "to" __ zone (__ "tapped"):? (__ "under" __ playersPossessive __ "control"):? (__ "attached to" __ object):?
    ...
    %}
  | "return" _s __ "to" __ zone __ object {% ([, , , , , to, , returns]) => ({ returns, to }) %}
  | "exile" _s __ object (__ fromZone):? (__ "face down"):?
  {%
    ([, , , exile, from, faceDown]) => {
      const result = { exile };
      if (from) result.from = from[1];
  ...
  %}
  | "create" _s __ tokenDescription {% ([, , , create]) => ({ create }) %}
  | ("copy" | "copies") __ object (__ countableCount):?
    {% ([, , copy, times]) => (times ? { copy, times: times[1] } : { copy }) %}
  ...
  | "remove" _s __ countableCount __ (counterKind __):? "counter" _s __ "from" __ object
    {%
      ([, , , count, , counterKind, , , , , , removeCountersFrom]) =>
        counterKind
          ? { count, removeCountersFrom, counterKind: counterKind[0] }
          : { count, removeCountersFrom }
    %}
  | castOrPlay _s __ "additional" __ object
    {% ([cp, , , , , cast]) => ({ [cp.toLowerCase()]: cast, additional: true }) %}
  | castOrPlay _s __ object __ fromZone __ "without paying its mana cost"
    {%
      ([cp, , , cast, , from, ,]) => ({
        [cp.toLowerCase()]: cast,
        from,
    ...
    %}
  | castOrPlay _s __ object (__ "without paying" __ ("its" | playersPossessive) __ "mana cost" _s):? (__ "and" __ asThoughClause):? (__ duration):? (__ "only during" __ partOfTurn):? (__ "on each of" __ qualifiedPartOfTurn _s):?
    {%
      ([cp, , , cast, withoutPaying, asThough, duration, onlyDuring, each]) => {
  ...
  | "search" "es":? __ zone (__ "for" __ object):?
    {%
      ([, , , search, criteria]) =>
        criteria ? { search, criteria: criteria[3] } : search
    %}
  | "choose" _s __ (object {% ([o]) => o %} | "a" __ anyType __ "type" {% ([, , type]) => ({ type }) %} | "not to" __ imperative {% ([, , not]) => ({ not }) %} | "a" (__ "nonland"):? __ "card name" {% ([, nonland]) => nonland ? { cardName: { not: "land" } } : "cardName" %} | "a color" {% () => "color" %} | "a" __ object __ "of each basic land type" {% ([, , what]) => ({ what, ofEach: "basicLandType" }) %})
    {% ([, , , choose]) => ({ choose }) %}
  | "draw" _s __ ("a" __ "card" {% () => 1 %} | "an additional card" {% () => 1 %} | englishNumber __ "additional" __ "card" "s" {% ([n]) => ({ additional: n }) %} | englishNumber __ "card" "s" {% ([n]) => n %})
    {% ([, , , draw]) => ({ draw }) %}
  | "draw" _s __ "cards equal to" __ numberDefinition {% ([, , , , , draw]) => ({ draw }) %}
  | "draw" _s __ "more than" __ englishNumber __ "card" _s (__ "each" __ "turn"):?
    {%
      ([, , , , , max, , , , duration]) => ({
        draw: { max },
  ...
  | "prevent" __ damagePreventionAmount __ damageNoun __ (object __ "would deal" {% ([from]) => ({ from }) %} | "that would be dealt to" __ "and dealt by" __ anyEntity {% ([, , , , target]) => ({ to: target, by: target }) %} | "that would be dealt" (__ "to" __ anyEntity):? {% ([, to]) => to ? { to: to[3] } : { to: "any" } %}) (__ duration):?
    {%
      ([, , amount, , prevent, , to, duration]) => {
        const result = to ? { amount, prevent, ...to } : { amount, prevent };
        if (duration) result.duration = duration[1];
        return result;
      }
    %}
  | "put" _s __ englishNumber __ counterKind __ "counter" _s __ "on" __ object
    {%
      ([, , , amount, , counterKind, , , , , , , putOn]) => ({
        amount,
        counterKind,
        putOn,
      })
  ...
  | "reveal" _s __ (object | zone) (__ "at random" __ fromZone):?
    {%
      ([, , , [reveal], random]) =>
        random ? { random: true, from: random[3], reveal } : { reveal }
    %}
  | "put" _s __ object (__ fromZone):? __ intoZone (__ "tapped"):? (__ "and" __ object __ intoZone):? (__ "under" __ playersPossessive __ "control"):? (__ "instead of" __ intoZone):?
    {%
      ([, , , put, from, , into, tapped, additional, control, insteadOf]) => {
        let result = { put, into };
        if (from) result.from = from[1];
    ...
    %}
  | "have" __ object __ (objectInfinitive {% ([property]) => ({ property }) %} | objectVerbPhrase {% ([does]) => ({ does }) %})
    {% ([, , have, , property]) => ({ have, ...property }) %}
  | "have" __ player __ playerVerbPhrase {% ([, , actor, , does]) => ({ actor, does }) %}
  | "have your life total become" __ numberDefinition
    {% ([, , lifeTotalBecomes]) => ({ lifeTotalBecomes }) %}
  | "have" __ numericalComparison __ numberDefinition
  ...
  | "switch the power and toughness of" __ object __ untilClause
    {% ([, , switchPowerToughness, , until]) => ({ switchPowerToughness, until }) %}
  ...
  | "cast" __ numericalComparison __ "spell" __ duration
    {%
      ([, , comparison, , what, , duration]) => ({
        cast: { comparison, what, duration },
      })
    %}
...
modifiedPlayerVerbPhrase -> basePlayerVerbPhrase (__ playerVerbModifier):?
{%
  ([does, mod]) => {
    if (!mod) return does;
    return { does, ...mod[1] };
...
basePlayerVerbPhrase -> gains __ "life equal to" __ itsPossessive __ numericalCharacteristic
  {% ([, , , , whose, , value]) => ({ lifeGain: { whose, value } }) %}
  | controls __ ("no" __):? object
  ...
  | "attack" ("s" | "ed"):? (__ player):? (__ "with" __ numericalComparison __ "creatures"):? (__ duration):?
    ...
    %}
  | "may" __ imperative __ "rather than" __ imperative
    {% ([, , may, , , , ratherThan]) => ({ may, ratherThan }) %}
  ...
  | "may play" __ object __ "and cast" __ object __ fromZone
...
modifiedObjectVerbPhrase -> baseObjectVerbPhrase (__ forEachClause):? (__ durationOrDuring):? (", where x is" __ numberDefinition):?
  {%
    ([does, forEach, dur, whereX]) => {
      let result = does;
  ...
  %}
  | baseObjectVerbPhrase (__ durationOrDuring) (__ forEachClause)
  {%
    ([does, dur, forEach]) => {
...
baseObjectVerbPhrase -> ("was" | "is") __ object {% ([, , is]) => ({ is }) %}
  | hasOrHave __ acquiredAbility {% ([, , haveAbility]) => ({ haveAbility }) %}
  | hasOrHave __ "base power and toughness" __ pt (__ "and" __ ("are" | "is") __ anyType _s __ "in addition to" __ itsPossessive __ "other types"):?
    {%
      ([, , , , basePowerToughness, andAre]) =>
        andAre ? { basePowerToughness, additionalType: andAre[5] } : { basePowerToughness }
    %}
  | gains __ acquiredAbility {% ([, , gains]) => ({ gains }) %}
  | gets __ ptModification {% ([, , powerToughnessMod]) => ({ powerToughnessMod }) %}
  | "enter" _s (__ "the battlefield"):? __ "with" __ (englishNumber {% ([n]) => n %} | englishNumber __ "additional" {% ([additional]) => ({ additional }) %}) __ counterKind __ "counter" _s __ "on it" (__ forEachClause):?
    {%
      ([, , , , , , amount, , counterKind, , , , , , forEach]) => ({
        entersWith: forEach
          ? { amount, counterKind, forEach: forEach[1] }
          : { amount, counterKind },
      })
    %}
  | "enter" _s (__ "the battlefield"):? __ "with a number of" (__ "additional"):? __ counterKind __ "counters on it equal to" __ numberDefinition
    {%
      ([, , , , , additional, , counterKind, , , , amount]) => ({
        entersWith: additional
          ? { counterKind, amount, additional: true }
          : { counterKind, amount },
      })
    %}
  ...
  %}
  | "cost" __ cost __ "more to" __ ("cast" | "activate")
    {% ([, , costIncrease, , , , [action]]) => ({ costIncrease, action }) %}
...
isWhat -> color {% ([color]) => ({ color }) %}
  | object (__ "in addition to its other" (__ "colors"):? (__ "and"):? (__ "types"):?):?
...
becomesWhat -> "tapped" {% () => "tap" %}
  ...
  | "a" _n (__ pt):? (__ color):? __ anyType (__ "with base power and toughness" __ pt):? (__ "with" __ acquiredAbility):? (__ "in addition to its other types"):?
    {%
      ([, , size, color, , type, size2, withClause, inAddition]) => {
        const result = { type };
...
tokenDescription -> englishNumber (__ pt):? (__ color):? __ permanentTypeSpecifier __ "token" _s (__ withClause):? (__ "named" __ [^.]:+):? (__ ("that's" | "that are") __ tokenState):?
  {%
    ([amount, size, color, , type, , , , withClause, name, state]) => {
  ...
  %}
  | "a number of" (__ pt):? (__ color):? __ permanentTypeSpecifier __ "token" _s __ "equal to" __ numberDefinition (__ withClause):? (__ ("that's" | "that are") __ tokenState):?
    {%
      ([, size, color, , type, , , , , , , amount, withClause, state]) => {
        const result = { amount, type };
        if (size) result.size = size[1];
        if (color) result.color = color[1];
        if (withClause) result.with = withClause[1];
    ...
    %}
  | englishNumber __ "token" _s __ "that" SAXON __ "a copy of" __ object
...
numberDefinition -> itsPossessive __ numericalCharacteristic
  {% ([whose, , characteristic]) => ({ whose, characteristic }) %}
  | "the" __ ("total" __):? "number of" __ object {% ([, , , , , count]) => ({ count }) %}
  | "the greatest number of" __ object {% ([, , count]) => ({ greatest: { count } }) %}
  | "the total" __ numericalCharacteristic __ "of" __ object
...
withClauseInner -> numericalCharacteristic __ numericalComparison
  {% ([value, , comparison]) => ({ value, comparison }) %}
  | "the highest" __ numericalCharacteristic __ "among" __ object
    {% ([, , highest, , , , among]) => ({ highest, among }) %}
  | "converted mana costs" __ numericalNumber __ "and" __ numericalNumber
...
dealsWhat -> damageNoun __ "to" __ damageRecipient {% ([damage, , , , to]) => ({ ...damage, to }) %}
  | numberDefinition __ "damage to" __ damageRecipient __ "and" __ numberDefinition __ "damage to" __ damageRecipient
    {%
      ([amount1, , , , to1, , , , amount2, , , , to2]) => ({
        and: [
          { amount: amount1, damageTo: to1 },
          { amount: amount2, damageTo: to2 },
        ],
      })
    %}
  | numberDefinition __ "damage to" __ damageRecipient
    {% ([amount, , , , damageTo]) => ({ amount, damageTo }) %}
  | numberDefinition __ "damage" __ "instead" {% ([amount]) => ({ amount, instead: true }) %}
  | "damage equal to" __ numberDefinition __ "to" __ damageRecipient
    {% ([, , amount, , , , damageTo]) => ({ amount, damageTo }) %}
  | "damage to" __ damageRecipient __ "equal to" __ numberDefinition
    {% ([, , damageTo, , , , amount]) => ({ amount, damageTo }) %}
  | numberDefinition __ "damage" __ divideAmongDamageTargets
    {% ([amount, , , , divideAmong]) => ({ amount, divideAmong }) %}
  | "damage equal to" __ numberDefinition __  divideAmongDamageTargets
    {% ([, , amount, , divideAmong]) => ({ amount, divideAmong }) %}
...
permanentTypeSpecifier -> permanentTypeSpecifierInner (__ permanentTypeSpecifierInner):*
  {% ([t1, ts]) => ({ and: [t1, ...ts.map(([, t]) => t)] }) %}

anyType -> anyTypeInner (__ anyTypeInner):*
  {% ([t1, ts]) => (ts.length > 0 ? { and: [t1, ...ts.map(([, t]) => t)] } : t1) %}
  | anyTypeInner __ "or" __ anyTypeInner {% ([t1, , , , t2]) => ({ or: [t1, t2] }) %}
  | anyTypeInner __ ("and/or" {% () => "or" %} | "and" {% () => "and" %}) __ anyTypeInner
...
asThoughClause -> "as though" __ player __ ("had" | "have" | "were") __ acquiredAbility
  {% ([, , who, , , , had]) => ({ who, had }) %}
...
exceptClause -> "except the first one" __ player __ "draw in each of" __ playersPossessive __ partOfTurn _s
  {%
    ([, , who, , , , whose, , step]) => ({
      who,
      does: "draw",
...
manacost -> manaSymbol:+ {% ([mg]) => mg %}
  | object SAXON __ "mana cost" (__ "reduced by" __ manacost):?
    {%
      ([costOf, , , , reduced]) =>
        reduced ? { costOf, reducedBy: reduced[3] } : { costOf }
    %}
  | itsPossessive __ "mana cost" (__ "reduced by" __ manacost):?
    {%
      ([costOf, , , reduced]) =>
        reduced ? { costOf, reducedBy: reduced[3] } : { costOf }
    %}
...
qualifiedPartOfTurn -> turnQualification __ partOfTurn _s
  {% ([qualification, , partOfTurn]) => ({ qualification, partOfTurn }) %}
  | "the beginning of" __ turnQualification __ partOfTurn _s
    {%
      ([, , qualification, , partOfTurn]) => ({
